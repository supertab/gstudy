...
t, db_triangle=signal.Signal(sampling_count).triangle(2, 3)

y1=3*np.sin(2*np.pi*3*t+np.pi/3)
y2=2.2*np.sin(2*np.pi*5*t+np.pi/6)
y_12 = y1+y2
y = y_12+db_triangle
...
ax1.plot(t, y_12)
ax2.set_title('tri')
ax2.plot(t, db_triangle)
ax3.set_title('y1+y2+tri')
ax3.plot(t, y)
